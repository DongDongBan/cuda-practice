...
// 前置条件：*d_y 需要初始化为0
static __global__ void reduce_warpshfl(const real *d_x, real *d_y, const int N) {
    ...
    real y = s_y[tid];
    for (int i = 16; i; i >>= 1) {
        y += __shfl_down_sync(FULL_MASK, y, i);
    }    
...
extern __host__ real reduce5(const real *d_x, const int N) {
    ...
    reduce_warpshfl<<<GRID_SIZE, BLOCK_SIZE, sizeof(real) * BLOCK_SIZE>>>(d_x, d_y, N);
